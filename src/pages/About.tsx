          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 420px), 1fr))', gap: '80px', alignItems: 'center' }}>
            <motion.div {...fadeUp}>
              ...
              <p style={{ fontSize: '16px', lineHeight: '1.8', color: '#475569', marginBottom: '20px' }}>
                IGI Home Renovations was founded on a simple belief: homeowners and property investors deserve a contractor they can trust — one that delivers exceptional quality, shows up on time, and communicates clearly from day one to final walkthrough.
              </p>
              <p style={{ fontSize: '16px', lineHeight: '1.8', color: '#475569', marginBottom: '20px' }}>
                With over 10 years of experience in residential renovation, our team has completed 100+ projects across Florida, New Jersey, and Pennsylvania — from single-bathroom upgrades to complete property overhauls for fix-and-flip investors.
              </p>
            ...
            >
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                {[
                  { number: '100+', label: 'Projects Completed', icon: 'bi-house-check' },
                  { number: '3', label: 'States Licensed', icon: 'bi-geo-alt' },
                  { number: '10+', label: 'Years Experience', icon: 'bi-calendar-check' },
                  { number: '5★', label: 'Average Rating', icon: 'bi-star' },
                ].map(stat => (
                  <div key={stat.label} style={{
                    padding: '28px 24px', background: '#F5F7FA', borderRadius: '16px',
                    border: '1px solid rgba(17,19,42,0.06)', textAlign: 'center',
                  }}>
                    <i className={`bi ${stat.icon}`} style={{ fontSize: '24px', color: '#0E8FD8', marginBottom: '12px', display: 'block' }} />
                    <div style={{ fontFamily: 'Montserrat', fontWeight: '800', fontSize: '32px', color: '#11132A', lineHeight: '1' }}>{stat.number}</div>
                    <div style={{ fontSize: '13px', color: '#475569', marginTop: '6px' }}>{stat.label}</div>
                  </div>
                ))}
              </div>
            </motion.div>
